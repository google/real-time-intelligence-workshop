...
def train_automl_model(data_set, timestamp, develop_mode):
    ...
    model = job.run(
        ...
        export_evaluated_data_items_bigquery_destination_uri='{}:flights.automl_evaluated'.format(PROJECT),
...
def main():
    ...
    # create data set
    all_files = tf.io.gfile.glob('gs://{}/train/data/all*.csv'.format(BUCKET))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--bucket',
        help='Data will be read from gs://BUCKET/train/data and checkpoints will be in gs://BUCKET/train/trained_model',
        required=True
    )
    parser.add_argument(
        '--region',
